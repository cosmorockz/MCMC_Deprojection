'''
This Code calculates density for the non-deprojected data from normalization using the formula:
...
norm = np.loadtxt("norm_PROJCT.dat",unpack=True)
...
for i in range(len(norm)):
	temp = (4./3.)*np.pi*(-((r[n_r-1]**2-r[i+1]**2)**(3./2.))+((r[n_r-1]**2-r[i]**2)**(3./2.)))
	Vol.append(temp)
...
np.savetxt("density_PROJCT.dat",n_e)
